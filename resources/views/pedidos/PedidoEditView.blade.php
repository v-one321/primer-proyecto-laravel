...
@section('contenido')
    <div class="card">
        ...
        <form action="{{ url('/pedidos/'.$dato->id) }}" method="post">
            @method('PUT')
            @csrf
            <div class="card-body">
                ...
                <div class="row gy-3">
                    <div class="col-12 col-md-6">
                        ...
                        <select name="cliente_id" id="cliente_id" class="form-select">
                            ...
                            @foreach ($clientes as $item)
                                <option value="{{ $item->id }}" {{ $item->id  == $dato->cliente_id ? 'selected' : ''}}>{{ $item->nombre.' '.$item->apellido }}</option>
                            @endforeach
                        </select>
                        @error('cliente_id')
                    ...
                    <div class="col-12 col-md-6">
                        ...
                        <select name="pelicula_id" id="pelicula_id" class="form-select">
                            ...
                            @foreach ($peliculas as $item)
                                <option value="{{ $item->id }}" {{ $item->id  == $dato->pelicula_id ? 'selected' : ''}}>{{ $item->nombre.' - '.$item->genero?->nombre }}</option>
                            @endforeach
                        </select>
                        @error('pelicula_id')
                            <small class="text-danger">
                    ...
                    <div class="col-12 col-md-6">
                        ...
                        <input type="date" name="fecha_devolucion" id="fecha_devolucion" class="form-control" placeholder="Escriba..."
                            value="{{ old('fecha_devolucion', $dato->fecha_devolucion) }}">
                    ...
                    <div class="col-12 col-md-6">
                        ...
                        <textarea name="observacion" id="observacion" class="form-control">{{ old('observacion', $dato->observaciones) }}</textarea>
                        @error('observacion')
                            <small class="text-danger">
